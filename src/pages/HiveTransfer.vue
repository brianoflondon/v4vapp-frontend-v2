...
async function sendTransfer() {
  try {
    const keychain = typeof window !== "undefined" ? window.hive_keychain : null
    if (!keychain) {
      $q.notify(t("keychain_not_installed"))
      return
    }
    ...
    await new Promise((resolve) => {
      keychain.requestTransfer(
        formParamsAsObject.data.username,
        formParamsAsObject.data.to,
        formParamsAsObject.data.amount,
        formParamsAsObject.data.memo,
        formParamsAsObject.data.currency,
        (response) => {
          if (!response?.success) {
            $q.notify(response?.message || "Transfer failed")
          }
          resolve(response)
        },
        formParamsAsObject.data.enforce,
      )
    })
  } catch (error) {
