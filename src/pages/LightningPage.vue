  ...
  switch (method) {
    ...
    case "HiveKeychain":
      ...
      if (result.success) {
        ...
        const txId =
          result?.result?.id || result?.result?.tx_id || result?.id || null
        if (!txId) {
          const txNotFoundMessage = `${t("transaction")}: ${t("not_found")}`
          dInvoice.value.progress.push(t("payment_sent"))
          dInvoice.value.progress.push(txNotFoundMessage)
          notif({
            color: "warning",
            avatar: "/site-logo/v4vapp-logo.svg",
            timeout: 4000,
            message: txNotFoundMessage,
            position: "top",
          })
          return
        }
        checkHiveTransaction(result.data.username, txId, notif)
      } else {
        dInvoice.value.progress.push(result.message)
        q.notify({
          color: "negative",
