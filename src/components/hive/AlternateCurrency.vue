  ...
  if (["hive", "hbd", "sats"].includes(dest.toLowerCase())) {
    if (storeAPIStatus?.minMax) {
      ...
      if (dest === "SATS") {
        ...
        const range = storeAPIStatus.minMax[dest]
        if (!range) {
        ...
        const balanceCap = storeUser.balancesNum?.[dest.toLowerCase()]
        if (Number.isFinite(balanceCap)) {
          min = Math.min(min, balanceCap)
          max = Math.min(max, balanceCap)
        }
      }
      const diff = max - min
