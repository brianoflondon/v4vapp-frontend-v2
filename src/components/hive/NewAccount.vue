...
async function requestInvoice() {
  ...
  try {
    ...
    while (true) {
      try {
        resp = await apiLogin.post(`${API_BASE}account/create`, accountData)
        if (resp.status !== 429) {
          break
        }
      } catch (error) {
...
async function checkPayment(expiresAt) {
  try {
    const resp = await apiLogin.post(`${API_BASE}check_invoice`, {
...
async function handlePaid() {
  ...
  try {
    ...
    while (true) {
      try {
        resp = await apiLogin.post(
          `${API_BASE}account/create_complete`,
          accountData,
        )
        if (resp.status !== 429) {
          break
