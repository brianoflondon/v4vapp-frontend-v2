      ...
      >
        ...
        <q-item class="justify-center">
          <q-btn
            ...
            align="left"
            :color="evmButtonDisabled ? 'grey-9' : 'primary'"
            rounded
        ...
        <q-item class="justify-center">
          <q-btn
            style="width: 200px"
            :disable="btcButtonDisabled"
            :label="btcAddressLabel"
            align="left"
            :color="btcButtonDisabled ? 'grey-9' : 'primary'"
            rounded
...
import { useBTCLoginFlow, useShortBTCAddress } from "src/use/useBTC"
import {
...
const btcAddressLabel = ref("BTC Login")

async function loginBTC() {
  const loginResult = await useBTCLoginFlow()
  if (!loginResult?.account) {
    return
  }
  btcAddressLabel.value = useShortBTCAddress(loginResult.account)
}
