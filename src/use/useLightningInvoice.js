...
export async function useGetLightingHiveInvoice(
  ...
  try {
    ...
    if (
      !hiveAccname ||
      typeof hiveAccname !== "string" ||
      hiveAccname.includes("blob:") ||
      hiveAccname.includes("http") ||
      hiveAccname === "blob"
    ) {
      console.error(
        "[useGetLightingHiveInvoice] CRITICAL: Bad hiveAccname value passed to invoice endpoint. This is a state bug upstream (possible blob URL leak into account fields). Value was:",
        hiveAccname,
      )
