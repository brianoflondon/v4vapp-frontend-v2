...
export async function useKeepSats(
  ...
  try {
    ...
    if (
      detail === "Hive Key authorization failure" ||
      ...
      const isNetworkError =
        ...
        /network error|cors|failed/i.test(error?.message || "")
      Notify.create({
        message: isNetworkError
          ? "Network/CORS issue contacting API"
          : "Communication issues, try again soon",
        color: "negative",
        position: "bottom",
        timeout: 2000,
      })
